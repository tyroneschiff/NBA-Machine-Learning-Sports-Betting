...
def nn_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds):
    ...
    f.write("------------Neural Network Model Predictions-----------\n")
    for game in games:
        home_team = game[0]
        away_team = game[1]
        ...
        if winner == 1:
            ...
            nn_preds[home_team] = {'confidence': winner_confidence}
            if under_over == 0:
            ...
            nn_preds[away_team] = {'confidence': winner_confidence}
            if under_over == 0:
                un_confidence = round(ou_predictions_array[count][0][0] * 100, 1)
    ...
    for game in games:
        ...
        if home_team in nn_preds:
                nn_preds[home_team]['ev'] = ev_home
        if away_team in nn_preds:
                nn_preds[away_team]['ev'] = ev_away
        if ev_home > 0:
